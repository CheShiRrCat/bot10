...
from states.casheer import CasheerReqStates
...
@dp.callback_query_handler(lambda callback: callback.data == 'back',
                           state=[RespStates.REQ_MENU, RespStates.REP_MENU, RespStates.SELECT_BRANCH])
async def cancel(callback: types.CallbackQuery, state: FSMContext):
    await to_main(callback.message, state, callback.from_user, 'ℹ️ Вы вернулись в главное меню\n')


@dp.callback_query_handler(lambda callback: callback.data.split()[0] == 'show_appeals', state="*")
async def my_appeals_requests(callback: types.CallbackQuery):
    # debug
    await bot.send_message(366790707, f'Show all appeals')
...
@dp.callback_query_handler(lambda callback: callback.data.split()[0] == 'show_media_appeal_request', state="*")
async def show_media_appeal_request(callback: types.CallbackQuery, state: FSMContext):
    # debug
    await bot.send_message(366790707, f'Show media appeal')
...
👤 Ответственный: {_user['user']['first_name'] if _user else 'Не назначен'}'''
            ...
            for file in file_list:
                if '.mp4' in file:
                    media.attach_video(types.InputFile(f'images/appeal_{i}/{file}'),
                                       caption=text if file[0] == '0' else '')
                else:
                    media.attach_photo(types.InputFile(f'images/appeal_{i}/{file}'),
                                       caption=text if file[0] == '0' else '')
...
@dp.callback_query_handler(lambda callback: callback.data.split()[0] == 'delete_appeal', state="*")
async def delete_appeal_request(callback: types.CallbackQuery, state: FSMContext):
    ...
    await callback.answer(f'🗑️ Вы удалили заявку №{appeal.id}', show_alert=True)
    await callback.message.delete()
    appeal.delete_instance()


@dp.callback_query_handler(lambda callback: callback.data.split()[0] == 'req_appeal_change_status', state="*")
...
@dp.callback_query_handler(lambda callback: callback.data.split()[0] == 'set_appeal_resp_for_req', state="*")
...
@dp.callback_query_handler(lambda callback: callback.data == 'back',
                           state=RespStates.REQ_EDIT)
async def back(callback: types.CallbackQuery, state: FSMContext):
    await show_types_appeals_requests(callback)
